...
5. **Explore Scope Candidates** — List everything that could be in scope. Do not filter yet. Include features, data, UI, platform concerns, and cross-cutting concerns.

6. **Identify Missing Concerns** — What has the user not mentioned that likely matters? Consider: error handling, data migration, performance, observability, testing, platform-specific behavior, offline support, update mechanism.
...
11. **Check for Architecture Implications** — If the idea touches domain modeling, bounded contexts, or cross-crate concerns, flag that deeper analysis should happen during planning. Do not attempt full modeling here.
...
## Planning Handoff
{What the Planning skill needs to know.}

- **Suggested MVP target:** {e.g., 1.0, 1.1, 2.0}
- **Suggested priority:** {P0 / P1 / P2 / P3}
- **Suggested type:** {feature / enhancement}
- **Architecture analysis needed:** {yes/no — flag if deeper crate/layer modeling is required during planning}
...
- Do not attempt full domain modeling. Flag it for the Planning skill if needed.
