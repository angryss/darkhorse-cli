...
> **HARD RULE: You MUST NOT write any code, create any source files, or modify any files in `crates/`, `frontend/`, or `deployment/`. Your ONLY output is a structured discovery document in `openspec/changes/`. Any code generation during discovery is a violation.**
