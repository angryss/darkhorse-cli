...
| Location | Contains |
|----------|----------|
| `backend/bffs/contexts/<name>/domain/` | Contracts/interfaces ONLY (API client interfaces, message sender interfaces) |
| `backend/bffs/contexts/<name>/application/` | Command handlers (→ broker), query handlers (→ API clients) |
| `backend/bffs/contexts/<name>/infrastructure/` | REST API client implementations, message sender implementations, auth adapters |
| `backend/bffs/contexts/<name>/presentation/` | REST controllers (frontend-facing), auth middleware |
