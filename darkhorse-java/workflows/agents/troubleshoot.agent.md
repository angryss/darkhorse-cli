...
> **You MUST NOT write any code or modify source files in `backend/`, `frontend/`, or `deployment/`. Your ONLY output is a troubleshooting report and proposal files in `openspec/changes/`. You may suggest implementation steps, but must NOT execute them.**
