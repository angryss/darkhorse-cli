...
describe('PivotTable', () => {
  ...
  describe('Rendering', () => {
    it('should render pivot table with data', () => {
      render(<PivotTable data={sampleData} config={basicConfig} />);
    ...
    it('should render empty state when no data', () => {
      render(<PivotTable data={[]} config={basicConfig} />);
    ...
    it('should render with custom dimensions', () => {
      const { container } = render(
        <PivotTable data={sampleData} config={basicConfig} height={500} width="80%" />
      );
      
      const pivotTable = container.firstChild as HTMLElement;
    ...
    it('should render with custom className', () => {
      const { container } = render(
        <PivotTable data={sampleData} config={basicConfig} className="custom-pivot" />
      );
  ...
  describe('Aggregations', () => {
    it('should calculate sum aggregation correctly', () => {
      const { container } = render(<PivotTable data={sampleData} config={basicConfig} />);
    ...
    it('should calculate average aggregation', () => {
      ...
      render(<PivotTable data={sampleData} config={avgConfig} />);
    ...
    it('should calculate count aggregation', () => {
      ...
      render(<PivotTable data={sampleData} config={countConfig} />);
      
      expect(screen.getByRole('grid')).toBeInTheDocument();
    });

    it('should calculate min/max aggregation', () => {
      const minMaxConfig: PivotConfig = {
        rows: [{ field: 'region' }],
        columns: [{ field: 'product' }],
        values: [{ field: 'sales', aggregation: 'max' }],
      };
      
      render(<PivotTable data={sampleData} config={minMaxConfig} />);
  ...
  describe('Toolbar', () => {
    ...
    it('should hide toolbar when showToolbar is false', () => {
      render(<PivotTable data={sampleData} config={basicConfig} showToolbar={false} />);
      
      expect(screen.queryByText('Pivot Table')).not.toBeInTheDocument();
    });

    it('should show export CSV button when enabled', () => {
      render(<PivotTable data={sampleData} config={basicConfig} allowCsvExport={true} />);
      
      expect(screen.getByText('Export CSV')).toBeInTheDocument();
    });

    it('should show export Excel button when enabled', () => {
      render(<PivotTable data={sampleData} config={basicConfig} allowExcelExport={true} />);
  ...
  describe('Cell Interaction', () => {
    it('should call onCellClick when a cell is clicked', async () => {
      ...
      const { container } = render(
        <PivotTable data={sampleData} config={basicConfig} onCellClick={onCellClick} />
      );
    ...
    it('should show drill-through dialog when onDrillThrough is provided', async () => {
      ...
      const { container } = render(
        <PivotTable data={sampleData} config={basicConfig} onDrillThrough={onDrillThrough} />
      );
  ...
  describe('Filters', () => {
    it('should apply include filters', () => {
      const filteredConfig: PivotConfig = {
        rows: [{ field: 'region' }],
        columns: [{ field: 'product' }],
      ...
      render(<PivotTable data={sampleData} config={filteredConfig} />);
  ...
  describe('Error Handling', () => {
    it('should call onError when an error occurs', () => {
      ...
      render(<PivotTable data={sampleData} config={invalidConfig} onError={onError} />);
