 ...
 */
export function getChartColors(count, theme) {
    const lightPalette = [
        '#3b82f6', // blue
        '#10b981', // green
        '#f59e0b', // amber
 ...
 */
export function calculateScale(min, max, tickCount = 5) {
    const range = max - min;
 ...
 */
export function generateSplinePath(points) {
    if (points.length < 2)
        return '';
    const p0 = points[0];
    const p1 = points[1];
 ...
 */
export function calculatePieSlicePath(centerX, centerY, radius, innerRadius, startAngle, endAngle) {
 ...
 */
export function calculatePieLabelPosition(centerX, centerY, radius, angle, offset = 20) {
