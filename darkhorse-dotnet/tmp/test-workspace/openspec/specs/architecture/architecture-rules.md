...
| **Application** | `Namespace.Application` | Domain + Common (NuGet) |
| **Infrastructure** | `Namespace.Infrastructure` | Application, Domain + Common (NuGet) |
| **Presentation** | `Namespace.Presentation` | Application, Infrastructure + Common (NuGet) |

> **`Common`** is consumed as an external NuGet package (`Namespace.Common`). It is never a local project. See Rule 14.
...
## Rule 14: Common Package

`Common` is not scaffolded as a local project. It is consumed as a NuGet package (`Namespace.Common`).

**What `Common` contains:**
- Shared DTOs and API contracts used across multiple services
- Cross-service primitives (pagination, error envelopes, correlation IDs)
- Shared value objects with no domain-specific logic

**Why it is a NuGet package, not a local project:**
- Ensures all services consume the same contract definitions without diverging copies
- Versioned independently — services opt into changes deliberately (`Version="1.0.*"`)
- Prevents accidental tight coupling between services through shared source
- Matches the Published Language context mapping pattern from DDD

**What `Common` MUST NOT contain:**
- Domain logic, business rules, or domain-specific entities
- Infrastructure code (no EF Core, no HTTP clients)
- Service-specific DTOs — only contracts shared by two or more services
