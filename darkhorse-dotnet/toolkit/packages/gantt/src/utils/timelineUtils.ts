 ...
 */
export function getTimelineScale(viewMode: TimelineViewMode): TimelineScale {
  const scales: Record<TimelineViewMode, TimelineScale> = {
    Day: {
      majorUnit: 'Week',
      minorUnit: 'Day',
      majorFormat: 'MMM DD',
...
): Array<{ date: Date; label: string; type: 'major' | 'minor' }> {
  const cells: Array<{ date: Date; label: string; type: 'major' | 'minor' }> = [];
 ...
 */
export function formatTimelineLabel(date: Date, viewMode: TimelineViewMode): string {
  switch (viewMode) {
    case 'Day':
...
export function calculateTimelineBounds(
  tasks: Array<{ startDate?: string | Date; endDate?: string | Date }>,
  projectStart?: string | Date,
  projectEnd?: string | Date
): { start: Date; end: Date } {
  let minDate: Date | null = projectStart ? new Date(projectStart) : null;
  let maxDate: Date | null = projectEnd ? new Date(projectEnd) : null;
