...
export const RichTextEditorExtended = forwardRef((props, ref) => {
    const { value, onChange, placeholder = 'Start typing...', showCharCount = false, readOnly = false, height = '400px', width = '100%', enableXhtml = true, slashMenuSettings, mentionSettings, quickToolbarSettings, onError, className, style, } = props;
    ...
    const [mentionMenu, setMentionMenu] = useState({ isOpen: false, position: { top: 0, left: 0 }, query: '', items: [], isLoading: false });
    const [quickToolbar, setQuickToolbar] = useState({ isOpen: false, target: 'text', position: { top: 0, left: 0 } });
    ...
    const handleMouseUp = useCallback(() => {
        updateToolbarState();
        if (!quickToolbarSettings || !editorRef.current)
            return;
        const selection = window.getSelection();
    ...
    const handleInsertMedia = (url, type) => {
    ...
    const handleMentionSelect = (item) => {
        const mentionHtml = mentionSettings?.displayTemplate
            ...
            : `<span data-mention-id="${item.id}" style="color: ${item.color || '#3b82f6'}; background-color: ${item.backgroundColor || '#dbeafe'}; padding: 2px 6px; border-radius: 4px;">@${item.name}</span>`;
    ...
    const handleQuickToolbarCommand = (command) => {
        execCmd(command);
        setQuickToolbar(prev => ({ ...prev, isOpen: false }));
    };
    // Format change
