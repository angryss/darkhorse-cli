...
describe('KanbanBoard', () => {
    ...
    describe('Rendering', () => {
        it('should render the Kanban board', () => {
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns }));
        ...
        it('should render cards in correct columns', () => {
            const { container } = render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns }));
            // Get all columns
    ...
    describe('Card Template', () => {
        ...
        it('should render custom card template', () => {
            const customTemplate = (card) => (_jsxs("div", { children: ["Custom: ", card.title] }));
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, cardTemplate: customTemplate }));
    ...
    describe('Column Toggle', () => {
        it('should show toggle buttons when allowColumnToggle is true', () => {
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, allowColumnToggle: true }));
        ...
        it('should toggle column when button is clicked', () => {
            ...
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, allowColumnToggle: true, onColumnToggle: onColumnToggle }));
            const toggleButtons = screen.getAllByRole('button', { name: /collapse/i });
    ...
    describe('Card Selection', () => {
        it('should not select cards when selectionMode is None', () => {
            ...
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, selectionMode: "None", onSelectionChange: onSelectionChange }));
        ...
        it('should select single card when selectionMode is Single', () => {
            const onSelectionChange = vi.fn();
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, selectionMode: "Single", onSelectionChange: onSelectionChange }));
            const card = screen.getByText('Task 1').closest('[role="button"]');
            if (card) {
                fireEvent.click(card);
            }
        ...
        it('should select multiple cards when selectionMode is Multiple', () => {
            const onSelectionChange = vi.fn();
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, selectionMode: "Multiple", onSelectionChange: onSelectionChange }));
    ...
    describe('Drag and Drop', () => {
        it('should make cards draggable when allowDragAndDrop is true', () => {
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, allowDragAndDrop: true }));
        ...
        it('should call onCardDrop when card is dropped', () => {
            ...
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, allowDragAndDrop: true, onCardDrop: onCardDrop }));
            const card = screen.getByText('Task 1').closest('[draggable]');
    ...
    describe('Swimlanes', () => {
        it('should render swimlanes when swimlaneSettings is provided', () => {
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, swimlaneSettings: {
                    keyField: 'assignee',
                } }));
            expect(screen.getByText('John')).toBeInTheDocument();
    ...
    describe('Empty States', () => {
        it('should show empty message when column has no cards', () => {
            const emptyCards = [];
            render(_jsx(KanbanBoard, { cards: emptyCards, columns: mockColumns }));
    ...
    describe('Card Click', () => {
        it('should call onCardClick when card is clicked', () => {
            ...
            render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, onCardClick: onCardClick }));
            const card = screen.getByText('Task 1').closest('[role="button"]');
            if (card) {
                fireEvent.click(card);
            }
    ...
    describe('Props Validation', () => {
        it('should use custom keyField', () => {
            ...
            render(_jsx(KanbanBoard, { cards: customCards, columns: mockColumns, keyField: "custom-status" }));
            expect(screen.getByText('Task')).toBeInTheDocument();
        });
        it('should render with custom height and width', () => {
            const { container } = render(_jsx(KanbanBoard, { cards: mockCards, columns: mockColumns, height: 600, width: "100%" }));
