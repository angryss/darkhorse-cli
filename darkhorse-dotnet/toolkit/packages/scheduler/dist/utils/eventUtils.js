 ...
 */
export function filterEventsByResource(events, resourceId) {
    return events.filter(event => {
        if (!event.resourceId)
            return false;
    ...
    sortedEvents.forEach(event => {
        ...
        for (const group of groups) {
            const canPlace = group.every(existingEvent => {
                ...
                return !eventsOverlap({ start: eventStart, end: eventEnd }, { start: existingStart, end: existingEnd });
            });
            if (canPlace) {
                group.push(event);
                placed = true;
 ...
 */
export function calculateEventLayout(events, dayStart, dayEnd, _slotHeight) {
