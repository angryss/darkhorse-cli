...
import { parseDate, formatTime, getTimeSlots, startOfWeek, endOfWeek, addDays, parseTime, isToday, isWorkDay, } from '../utils/dateUtils';
import { filterEventsByDateRange, calculateEventLayout, isAllDay } from '../utils/eventUtils';
import styles from '../Scheduler.module.css';
export const WeekView = ({ currentDate, events, workDays = [1, 2, 3, 4, 5], firstDayOfWeek = 0, dayStartHour = '00:00', dayEndHour = '23:59', workStartHour = '09:00', workEndHour = '18:00', timeScale = { majorSlot: 60, minorSlotCount: 2 }, timeFormat = 'hh:mm a', eventTemplate, dateHeaderTemplate, onEventClick, onSlotClick, }) => {
    ...
    const handleSlotClick = (slotTime, day) => {
        if (onSlotClick) {
    ...
    return (_jsxs("div", { className: styles.time_grid, role: "grid", "aria-label": "Week view", children: [_jsxs("div", { className: styles.time_grid__header, children: [_jsx("div", { className: styles.time_grid__time_header, "aria-label": "Time", children: "Time" }), _jsx("div", { className: styles.time_grid__day_headers, children: weekDays.map((day, index) => (_jsx("div", { className: `${styles.time_grid__day_header} ${isToday(day) ? styles['time_grid__day_header--today'] : ''}`, children: dateHeaderTemplate ? (dateHeaderTemplate({ date: day })) : (_jsxs(_Fragment, { children: [_jsx("div", { className: styles.time_grid__day_header__day, children: day.toLocaleDateString('en-US', { weekday: 'short' }) }), _jsx("div", { className: styles.time_grid__day_header__date, children: day.getDate() })] })) }, index))) })] }), _jsxs("div", { className: styles.time_grid__body, children: [_jsx("div", { className: styles.time_grid__time_column, children: timeSlots.map((slot, index) => (_jsx("div", { className: styles.time_grid__time_slot, style: { height: 60 / timeScale.minorSlotCount }, children: slot.getMinutes() === 0 && formatTime(slot, timeFormat) }, index))) }), _jsx("div", { className: styles.time_grid__days, children: eventsByDay.map(({ day, layout }, dayIndex) => {
                            const currentTimePosition = getCurrentTimePosition(day);
                            const isWeekendDay = !isWorkDay(day, workDays);
                            return (_jsxs("div", { className: `${styles.time_grid__day} ${isWeekendDay ? styles['time_grid__day--weekend'] : ''}`, children: [_jsx("div", { className: styles.time_grid__day__slots, children: timeSlots.map((slot, slotIndex) => (_jsx("div", { className: `${styles.time_grid__slot} ${isWorkHour(slot) && !isWeekendDay
                                                ? styles['time_grid__slot--work_hours']
                                                : styles['time_grid__slot--non_work_hours']}`, style: { height: 60 / timeScale.minorSlotCount }, onClick: () => handleSlotClick(slot, day), role: "gridcell", tabIndex: 0, onKeyDown: (e) => {
                                                if (e.key === 'Enter' || e.key === ' ') {
                                                    e.preventDefault();
                                                    handleSlotClick(slot, day);
                                                }
                                            } }, slotIndex))) }), currentTimePosition !== null && (_jsx("div", { className: styles.time_grid__current_time, style: { top: `${currentTimePosition}%` }, "aria-label": "Current time" })), _jsx("div", { className: styles.time_grid__events, children: layout.map(({ event, top, height, left, width }) => {
                                            const eventStart = parseDate(event.start);
                                            const eventEnd = parseDate(event.end);
                                            return (_jsx("div", { className: `${styles.time_grid__event} ${event.isReadonly ? styles['time_grid__event--readonly'] : ''} ${event.recurrenceRule ? styles['time_grid__event--recurring'] : ''}`, style: {
                                                ...
                                                }, children: eventTemplate ? (eventTemplate(event)) : (_jsxs(_Fragment, { children: [_jsx("div", { className: styles.time_grid__event__subject, children: event.subject }), _jsx("div", { className: styles.time_grid__event__time, children: formatTime(eventStart, timeFormat) })] })) }, event.id));
                                        }) })] }, dayIndex));
                        }) })] })] }));
};
