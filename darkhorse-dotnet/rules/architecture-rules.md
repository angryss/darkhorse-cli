...
|-------|---------|------------|
| **Domain** | `Namespace.Domain` | **NONE** — zero external dependencies |
| **Application** | `Namespace.Application` | Domain + `<WorkspaceNs>.Contracts` (ProjectReference) |
| **Infrastructure** | `Namespace.Infrastructure` | Application, Domain + `<WorkspaceNs>.Contracts` (ProjectReference) |
| **Presentation** | `Namespace.Presentation` | Application, Infrastructure (`Contracts` transitive) |
| **Contracts** | `<WorkspaceNs>.Contracts` | **NONE** — zero dependencies |

> **`<WorkspaceNs>.Contracts`** is a workspace-level class library at `backend/shared/`. It is **NOT** a NuGet package — services reference it via relative `ProjectReference`. It contains only the published language shared across service boundaries. See Rule 14.
...
> **MediatR is the required and only approved CQRS dispatch mechanism.** Do not implement a custom mediator. Inject `ISender` (not `IMediator`) in controllers and consumers. Use `IPipelineBehavior<T, R>` for cross-cutting concerns (logging, validation, transactions).

### Commands (Writes)
- Implement `IRequest<T>` (MediatR)
- Modify state
- Return void or ID
- MAY publish integration events AFTER successful persistence
...
- MUST NOT publish events or trigger side effects
...
| Constraint | Rule |
|-----------|------|
| **No persistence** | BFF MUST NOT reference EF Core, Npgsql, or any database driver |
| **No domain logic** | BFF Domain layer contains interfaces and contracts ONLY |
| **No repositories** | BFF MUST NOT define or implement repository interfaces |
| **Queries → downstream APIs** | All BFF queries MUST delegate to typed `IApiClient` implementations via HttpClient |
| **Commands → broker** | All BFF commands MUST delegate to typed `IMessageSender` implementations via MassTransit |
| **No direct data access** | All data originates from downstream services, never from a local store |

### BFF Violations

```
❌ BFF Application layer containing repository usage
❌ BFF Infrastructure containing EF Core / Npgsql
❌ BFF Domain containing entities or aggregates
❌ BFF Application publishing domain events (dispatches commands only)
❌ BFF writing to any local database
```
...
| Category | Examples |
|----------|----------|
| **Integration events** | `OrderPlacedEvent`, `PaymentReceivedEvent` (past-tense domain actions) |
| **Cross-service message contracts** | Command/event message payloads published to the broker |
| **Shared value types / primitives** | `CorrelationId`, `PaginatedResult<T>`, `ErrorEnvelope` |
| **True cross-service abstractions** | Interfaces shared by two or more services (e.g. `ICorrelatable`) |

### What `Contracts` MUST NOT contain

> ⛔ Adding the following to `Contracts` violates service autonomy.

| Forbidden | Why |
|-----------|-----|
| **Service-specific DTOs** | DTOs belong in the service's `Application` layer |
| **Domain entities or aggregates** | Domain models belong in the service's `Domain` layer |
| **Repository interfaces** | Repository interfaces belong in the service's `Domain` layer |
| **EF models or persistence concerns** | Persistence belongs in the service's `Infrastructure` layer |
| **Controller/request/response models** | Request/response models belong in the service's `Presentation` layer |
| **Generic helper or utility classes** | Shared utilities do not belong in published language contracts |

### Why this boundary matters

- **Service autonomy** — each service owns and evolves its internal types independently
- **Published Language** — `Contracts` is a deliberate, versioned API surface between services, not a dumping ground
- Putting service-internal concerns in `Contracts` creates invisible coupling that undermines the bounded context model
- If only one service uses a type, it belongs inside that service — period

### `Contracts` has zero dependencies

The `Contracts` project MUST NOT reference any NuGet package beyond the .NET SDK itself. No MediatR, no EF Core, no ASP.NET Core types.
