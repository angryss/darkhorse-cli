...
Run `darkhorse-dotnet discover --input <discover-input.json> --json`, then ask:
...
Run `darkhorse-dotnet plan --input <plan-input.json> --json`, then ask:
...
Run `darkhorse-dotnet implement <change-id> --json`, then ask:

> Implement only the approved A1 scope using bounded contexts, ASP.NET Core services, Clean Architecture, CQRS, topology, and toolkit reuse. Do not persist separate lifecycle state.

## Test

Run `darkhorse-dotnet test --input <proof-input.json> --json`. When independent review is required, run `darkhorse-dotnet review --input <review-input.json> --json` as part of the Test stage.

> Help diagnose failed proof without changing the expected result or manufacturing success.

## Close

Run `darkhorse-dotnet close --input <close-input.json> --json`, then follow the exact VEP next action.
