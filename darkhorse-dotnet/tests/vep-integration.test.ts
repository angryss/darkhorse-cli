...
async function projectRoot(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'dh-dotnet-vep-integration-'));
  roots.push(root);
  return root;
}
...
describe('terminal VEP integration boundary', () => {
  it('admits only the frozen public VEP identity and exact version', () => {
    expect(evaluateVepCompatibility({
      darkhorseVersion: '0.1.0', packageName: '@angryss/vep', versionSpec: '2.0.0',
  ...
  it('treats root package.json as the sole current VEP version authority', async () => {
    ...
    await writeFile(path.join(root, 'package.json'), JSON.stringify({
      name: 'generated-dotnet', version: '0.1.0', private: true,
      devDependencies: { '@angryss/vep': '2.0.0' },
    }));
    await expect(readProjectVepSelection(root)).resolves.toMatchObject({
      projectRoot: root, packageName: '@angryss/vep', version: '2.0.0',
    });
  });
