...
async function fixture(packageJson: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'dh-dotnet-vep-negative-'));
  roots.push(root);
...
describe('terminal VEP fail-closed boundary', () => {
  it.each(['latest', '^2.0.0', '~2.0.0', '*', 'file:../vep', 'https://example.test/vep.tgz'])(
    'rejects floating or non-public selection %s without mutation', (versionSpec) => {
      expect(evaluateVepCompatibility({ darkhorseVersion: '0.1.0', packageName: '@angryss/vep', versionSpec }))
        .toMatchObject({ supported: false, code: 'NON_EXACT_VEP_VERSION' });
    },
  );
  it('rejects an unsupported exact release', () => {
    expect(evaluateVepCompatibility({ darkhorseVersion: '0.1.0', packageName: '@angryss/vep', versionSpec: '2.0.1' }))
      .toMatchObject({ supported: false, code: 'UNSUPPORTED_VEP_VERSION' });
  });
  ...
  it('returns governed recovery with every boundary error', async () => {
    ...
    try { await readProjectVepSelection(root); throw new Error('expected failure'); }
      ...
      expect(error).toBeInstanceOf(ProjectVepBoundaryError);
      expect((error as ProjectVepBoundaryError).recovery.length).toBeGreaterThan(20);
    }
  });
});
