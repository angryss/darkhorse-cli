...
function planAheadInput(overrides: Partial<NxAnalysisInput> = {}): NxAnalysisInput {
  return {
    ...
    ecosystem: 'dotnet',
    intendedProjects: [
      { name: 'web-api', type: 'app', technology: 'dotnet', description: 'API service' },
      { name: 'worker', type: 'app', technology: 'dotnet', description: 'Background worker' },
      { name: 'shared-lib', type: 'lib', technology: 'dotnet', description: 'Shared library' },
    ],
    scalePrediction: {
      expectedApps: 3,
      expectedLibs: 2,
...
function migrationInput(overrides: Partial<NxAnalysisInput> = {}): NxAnalysisInput {
  return {
    ...
    currentStructure: {
      rootPath: '.',
      projects: [
        { name: 'api-gateway', path: 'src/api-gateway', type: 'app', technology: 'dotnet', buildTool: 'dotnet' },
        { name: 'order-service', path: 'src/order-service', type: 'app', technology: 'dotnet', buildTool: 'dotnet' },
        { name: 'inventory-service', path: 'src/inventory-service', type: 'app', technology: 'dotnet', buildTool: 'dotnet' },
        { name: 'shared-contracts', path: 'src/contracts', type: 'lib', technology: 'dotnet', buildTool: 'dotnet' },
      ],
      sharedAssets: ['src/contracts'],
      buildCommands: { build: 'dotnet build', test: 'dotnet test' },
    },
...
describe('nx-monorepo skill: recommendation correctness', () => {
  ...
  it('low-fit scenario returns not-recommended', async () => {
    const result = await analyzeNxMonorepo(planAheadInput({
      intendedProjects: [
        { name: 'single-app', type: 'app', technology: 'dotnet', description: 'Only app' },
      ],
      scalePrediction: {
        expectedApps: 1,
        expectedLibs: 0,
  ...
  it('medium-fit scenario returns optional', async () => {
    const result = await analyzeNxMonorepo(planAheadInput({
      intendedProjects: [
        { name: 'api', type: 'app', technology: 'dotnet', description: 'API' },
        { name: 'web', type: 'app', technology: 'dotnet', description: 'Web' },
      ],
      scalePrediction: {
        expectedApps: 2,
        expectedLibs: 1,
...
describe('nx-monorepo skill: plan-ahead mode', () => {
  ...
  it('includes task orchestration', async () => {
    const result = await analyzeNxMonorepo(planAheadInput());
...
describe('nx-monorepo skill: ecosystem specifics', () => {
  it('dotnet ecosystem includes dotnet-specific plugins and tasks', async () => {
    const result = await analyzeNxMonorepo(planAheadInput({ ecosystem: 'dotnet' }));
  ...
  it('java ecosystem includes java-specific plugins and tasks', async () => {
    const result = await analyzeNxMonorepo(planAheadInput({
      ecosystem: 'java',
      intendedProjects: [
        { name: 'order-svc', type: 'app', technology: 'java', description: 'Order service' },
        { name: 'user-svc', type: 'app', technology: 'java', description: 'User service' },
        { name: 'common', type: 'lib', technology: 'java', description: 'Shared' },
      ],
    }));
