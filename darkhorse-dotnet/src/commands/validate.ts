  ...
  program
    ...
    .action(async (opts) => {
      ...
      if (!(await configExists(projectRoot))) {
        ...
        logger.info('Run `darkhorse-dotnet init` first to initialize a project.');
        process.exit(1);
      }
      ...
      logger.header('DarkHorse — Validate Project');
      logger.blank();
      ...
      if (config.ai.tools.copilot) {
        ...
        for (const check of copilotChecks) {
          if (!(await fsUtil.pathExists(check.path))) {
            logger.error(`  Missing: ${check.label}`);
            logger.info(`    Fix: darkhorse-dotnet ai sync --tools copilot`);
            hasErrors = true;
          } else {
            logger.success(`  OK: ${check.label}`);
          }
      ...
      if (config.ai.tools.kiro) {
        ...
        for (const filename of KIRO_STEERING_FILES) {
          ...
          if (!(await fsUtil.pathExists(filePath))) {
            ...
            logger.info(`    Fix: darkhorse-dotnet ai sync --tools kiro`);
            hasErrors = true;
          } else {
        ...
        logger.info('Kiro not enabled (ai.tools.kiro = false). Run `darkhorse-dotnet ai sync --tools kiro` to add Kiro support.');
      }
