 ...
 *
 * @param config  Project config (ai.tools flags already updated by the command)
 * @param tools   List of tool names requested (e.g. ['kiro', 'copilot'])
 * @param force   When true, overwrite existing adapter files
 */
...
async function syncCopilotAdapter(
  ...
  const agentFiles = await syncMarkdownDir(
    path.join(getWorkflowsDir(), 'agents'),
    githubAgentsDir,
    force,
  );
  ...
  const promptFiles = await syncMarkdownDir(
    path.join(getWorkflowsDir(), 'commands'),
    promptsDir,
    force,
  );
...
/**
 * Idempotent markdown sync: copies .md files from srcDir to destDir,
 * skipping files that already exist unless force=true.
 */
