...
export async function scaffoldWorkspace(config: DarkhorseConfig): Promise<SkillResult> {
  ...
  try {
    ...
    // Frontend package.json files + toolkit copy (optional)
    if (config.features.frontend) {
      const platforms = config.frontend?.platform === 'both'
        ? ['web', 'mobile'] as const
        : [config.frontend?.platform ?? 'web'] as const;
...
export async function scaffoldService(config: DarkhorseConfig): Promise<SkillResult> {
  const filesCreated: string[] = [];
  const errors: string[] = [];
  const p = config.paths;
...
async function scaffoldCqrsExamples(
  ...
  if (archetype === 'api' || archetype === 'microservice') {
    ...
    const domainFiles = [
      ...
      ...(archetype === 'api' ? [['ExampleValueObject.cs.hbs', 'ExampleValueObject.cs'] as [string, string]] : []),
    ];
