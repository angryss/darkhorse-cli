...
export async function scaffoldProject(config) {
    ...
    try {
        ...
        // Application crate
        await engine.render('backend/application/Cargo.toml.hbs', ctx, path.join(crates, `${prefix}-application`, 'Cargo.toml'));
        await engine.render('backend/application/lib.rs.hbs', ctx, path.join(crates, `${prefix}-application`, 'src', 'lib.rs'));
        filesCreated.push(`crates/${prefix}-application/Cargo.toml`, `crates/${prefix}-application/src/lib.rs`);
        // Infrastructure crate
        await engine.render('backend/infrastructure/Cargo.toml.hbs', ctx, path.join(crates, `${prefix}-infrastructure`, 'Cargo.toml'));
        await engine.render('backend/infrastructure/lib.rs.hbs', ctx, path.join(crates, `${prefix}-infrastructure`, 'src', 'lib.rs'));
        filesCreated.push(`crates/${prefix}-infrastructure/Cargo.toml`, `crates/${prefix}-infrastructure/src/lib.rs`);
        // Desktop shell crate
        await engine.render('backend/desktop/Cargo.toml.hbs', ctx, path.join(crates, `${prefix}-desktop`, 'Cargo.toml'));
        await engine.render('backend/desktop/main.rs.hbs', ctx, path.join(crates, `${prefix}-desktop`, 'src', 'main.rs'));
        await engine.render('backend/desktop/build.rs.hbs', ctx, path.join(crates, `${prefix}-desktop`, 'build.rs'));
        filesCreated.push(`crates/${prefix}-desktop/Cargo.toml`, `crates/${prefix}-desktop/src/main.rs`, `crates/${prefix}-desktop/build.rs`);
        ...
        if (config.features.frontend) {
            ...
            filesCreated.push('frontend/package.json', 'frontend/tsconfig.json', 'frontend/vite.config.ts', 'frontend/index.html', 'frontend/src/main.ts');
        }
