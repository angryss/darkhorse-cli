...
const roots: string[] = [];
async function fixture(packageJson: unknown): Promise<string> { const root = await mkdtemp(path.join(os.tmpdir(), 'dh-desktop-vep-negative-')); roots.push(root); await writeFile(path.join(root, 'package.json'), JSON.stringify(packageJson)); return root; }
