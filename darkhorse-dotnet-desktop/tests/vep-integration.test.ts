...
const roots: string[] = [];
async function projectRoot(): Promise<string> { const root = await mkdtemp(path.join(os.tmpdir(), 'dh-desktop-vep-integration-')); roots.push(root); return root; }
...
describe('terminal VEP integration boundary', () => {
  ...
  it('treats root package.json as the sole current VEP version authority', async () => {
    ...
    await writeFile(path.join(root, 'package.json'), JSON.stringify({ name: 'generated-desktop', version: '0.1.0', private: true, devDependencies: { '@angryss/vep': '2.0.0' } }));
